...
from record.factory import build_record_handler
import transform
...
def write_values(stream_to_write):
    schema = stream_to_write['schema'].to_dict()
...
def sync(config, state, catalog):
    ...
    for model_recommendation in model_with_recommendations:
        LOGGER.info('Syncing recommendation with id: {}'.format(model_recommendation['id']))
